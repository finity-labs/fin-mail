...
- The static analysis gate failed on a fresh install with PHPStan 2.2.13, which dropped the `#1` position from the "expects view-string" message for named arguments. The ignore pattern in `phpstan.neon` now accepts both forms. No runtime change
